...
- **The panel** is a borderless, non-activating `NSPanel` (300pt wide, height fits content):
  ...
  - Frosted `NSVisualEffectView` (`.popover`) surface, 16pt continuous-corner radius, spring animations between states (response 0.35, damping 0.8). Clicking the running timer's digits previews the sheen wave. **Liquid Glass was tried and rejected** (July 2026): the material worked correctly — and that was the problem. Regular glass adapts its light/dark presentation to the backdrop by design (dark over Terminal, light over a browser), so the always-visible panel flipped personality with every window behind it. For a persistent ambient surface the constancy of the frosted material outweighs the beauty of the glass. A deliberate product decision, not a bug workaround — revisit only if the constancy principle itself is revisited.
