...
- **The panel** is a borderless, non-activating `NSPanel` (300pt wide, height fits content):
  ...
  - Frosted `NSVisualEffectView` (`.popover`) surface, 16pt continuous-corner radius, spring animations between states (response 0.35, damping 0.8). Clicking the running timer's digits previews the sheen wave. **Liquid Glass was tried and reverted** (July 2026, macOS 26.x): glass backdrop compositing proved unreliable for this window configuration (borderless + non-activating + transparent + floating + all-Spaces) — it worked when hot-swapped into a live panel but on cold boot rendered a brief light placeholder then collapsed to clear/black, in light mode, regardless of backdrop (SwiftUI `glassEffect`, `GlassEffectContainer`, and window-level `NSGlassEffectView` all failed identically). This looks like an OS bug, not designed behavior — worth retrying on a later macOS, with the acceptance test being a cold boot, not a hot swap.
