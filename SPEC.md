...
- **The panel** is a borderless, non-activating `NSPanel` (300pt wide, height fits content):
  ...
  - Draggable by its background; position persists across launches, clamped fully on-screen (a saved off-screen position self-heals on next launch).
  - Grows upward from its bottom-left anchor when content expands (bottom edge re-pinned on resize).
  - Vibrancy material background (`.popover`), 16pt continuous-corner radius, hairline border, spring animations between states (response 0.35, damping 0.8).
- An invisible main menu provides the Edit key equivalents (⌘A/⌘C/⌘V/⌘X/⌘Z/⇧⌘Z) that accessory apps otherwise lack.
- Launch at login via `SMAppService`, offered on by default at first run, toggleable in Settings.
...
Phases: `firstRun`, `picker`, `running`, `checkIn`, `pauseMenu`, `switchNote`, `onBreak`, `away`, `completion`, `parkingFull`.

- `firstRun` → (Let's go) → `picker`. One-time screen: hotkey mention, launch-at-login toggle, "connect Todoist later".
...
- `checkIn` → Yes or 60s timeout → `running` (backoff advances); Pausing → `pauseMenu`; idle/sleep → `away`.
...
| Breath | midpoint of current interval, once per interval |
...
| Store heartbeat | every 30 s while running |
...
Stats/charts, streaks, Pomodoro modes, multiple timers, iCloud sync, iOS companion, calendar integration, website/app blocking, sounds, onboarding beyond the first-run card, to-do lists.
